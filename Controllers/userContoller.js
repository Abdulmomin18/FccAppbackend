...
`
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
                ...
                res.status(200).json({ message: 'OTP sent via email. Please check your email.' });
            }
        });
    } catch (error) {
        console.log(error);
...
const registerUser = async (req, res) => {
    try {
        ...
        // Save OTP with timestamp
        const otpExpiresAt = new Date(Date.now() + 1 * 60 * 60 * 1000); // 1 hour from now
        user = new userModel({ name, email, phonenumber, password, otp, otpExpiresAt });
...
`
        ...
        res.status(200).json({ status: true, data: { _id: user._id, name, email, phonenumber, otp } });
    } catch (error) {
        console.log(error);
        res.status(500).json(error);
    }
...
const verifyOTP = async (req, res) => {
    ...
    try {
        ...
        if (userOtp.otp === otp && userOtp.otpExpiresAt > new Date()) {
            ...
            await UserOtp.updateOne({ email }, { $unset: { otp: "", otpExpiresAt: "" } });
...
module.exports = { registerUser, loginUser, findUser, getUsers, verifyOTP, sendEmail }